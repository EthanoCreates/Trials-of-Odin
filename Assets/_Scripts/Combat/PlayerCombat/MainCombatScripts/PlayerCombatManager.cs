...
public class PlayerCombatManager
{
    ...
    public PlayerCombatManager()
    {
        ...
        playerState.PlayerAnimationEvents.OnComboAvalible += (object sender, EventArgs e) => IsComboAvailable = true;
        playerState.PlayerAnimationEvents.OnComboUnAvalible += (object sender, EventArgs e) => IsComboAvailable = false;
        playerState.PlayerAnimationEvents.OnRage += PlayerAnimationEvents_OnRage;
    }

    private void PlayerAnimationEvents_OnRage(object sender, EventArgs e)
    {
        PlayerUI.Instance.RageUI(rageTime);
        TurnOffRageBuffAfterTime(rageTime);
    ...
    private void PlayerContext_OnNewActiveWeapon(object sender, WeaponHolster.WeaponEventArgs e)
    {
        Weapon = e.Weapon;
    ...
    private void WarpMotion(AnimationClip attackAnimation, float animationSpeed, bool positionalWarping)
    {
        ...
        warpPhase.tOffset = target.GetComponent<EnemyTarget>().targetParent.transform.position - PlayerStateMachine.LocalInstance.transform.forward.Flatten();
    ...
    public bool TryFinisherAttack()
    {
        ...
        if(target.GetComponent<EnemyTarget>().targetParent.TryGetComponent(out IHumanoidUtilities humanoidUtilities))
        {
            if (humanoidUtilities.Utilites.CombatManager.isFinisherable) { 
                ...
                CoroutineHelper.Instance.StartCoroutine(TurnOffJumpAfterDelay(.2f));
