...
public class WeaponHolster : MonoBehaviour
{
    ...
    public class WeaponEventArgs{ public Weapon Weapon; }
    public class EquippedWeaponType { public bool IsPrimaryWeapon; }

    public event EventHandler<WeaponEventArgs> OnNewActiveWeapon;

    //activated on input
    public event EventHandler <EquippedWeaponType> OnEquip;
    public event EventHandler <WeaponEventArgs> OnUnEquip;
    ...
    void Start()
    {
        ...
        //on inputs
        GameInput.Instance.OnEquipPrimary += (object sender, EventArgs e) => SwitchWeapons(true);
        GameInput.Instance.OnEquipSecondary += (object sender, EventArgs e) => SwitchWeapons(false);
    ...
    private void PlayerState_OnUnEquipShield(object sender, EventArgs e)
    {
        ...
        OnNewActiveWeapon?.Invoke(this, new WeaponEventArgs
        {
            Weapon = ActiveWeapon,
        });
    }

    private void PlayerState_OnEquipShield(object sender, EventArgs e)
    {
        if (shield == null) return;
        EquipShield();
    }
    ...
    private void Instance_OnEquipShield(object sender, EventArgs e)
    {
        if (shield == null) return;
    ...
    private void AnimationEvent_OnUnEquip(object sender, EventArgs e)
    {
    ...
    private void AnimationEvent_OnEquip(object sender, EventArgs e)
    {
        if (isPrimary && secondaryWeapon != null && !IsThrown(secondaryWeapon)) secondaryWeapon.WeaponToHolster();
        if (!isPrimary && primaryWeapon != null && !IsThrown(primaryWeapon)) primaryWeapon.WeaponToHolster();
    ...
    public void UnEquip()
    {
        OnUnEquip?.Invoke(this, new WeaponEventArgs
        {
            Weapon = ActiveWeapon,
        });
    }
    ...
    public void Equip(Weapon weapon)
    {
        ...
        if (weapon == primaryWeapon) isPrimary = true;

        OnEquip?.Invoke(this, new EquippedWeaponType
        {
            IsPrimaryWeapon = isPrimary
        });
    }
