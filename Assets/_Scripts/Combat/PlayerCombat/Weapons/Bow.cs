...
namespace TrialsOfOdin.Combat
{
    public class Bow : Weapon, IShootable, IAimAttacks
    {
        ...
        /// <param name="e"></param>
        private void PlayerState_OnReloadWeapon(object sender, System.EventArgs e)
        {
            loaded = true;
            SpawnArrow();
        }
        ...
        private IEnumerator TryReload()
        {
            ...
            while (!loaded)
            {
                PlayerStateMachine.EPlayerState currentState = playerState.currentState.StateKey;
                PlayerStateMachine.EPlayerState subState = playerState.currentState.CurrentSubState.StateKey;

                // Check if the player is in a valid state to reload
                if (currentState == PlayerStateMachine.EPlayerState.Grounded &&
                    (subState == PlayerStateMachine.EPlayerState.Idle ||
                     subState == PlayerStateMachine.EPlayerState.Walk ||
                     subState == PlayerStateMachine.EPlayerState.Run ||
                     subState == PlayerStateMachine.EPlayerState.Aim))
                {
                    if (!reloading && playerState.Context.AnimFinished)
                    {
            ...
            if (playerState.CombatManager.IsAiming) playerState.AnimationRequestor.AnimateAim();
        }
